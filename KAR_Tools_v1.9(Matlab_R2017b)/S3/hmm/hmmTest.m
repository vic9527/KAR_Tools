function [accuracy, predict_label, true_label] = hmmTest(TestData, HMM_Models, FVrange)

model_number = length(HMM_Models); % model number
test_number = length(TestData);
...
for i = 1:test_number
    % get true label
    true_label(i) = TestData(i).label;
    for j = 1:model_number
%                 data = TE_Actions(i).Observations;
%          data = TE_Actions(i).Observations(7:12,:);
        data = TestData(i).Feature(FVrange,:);% 
