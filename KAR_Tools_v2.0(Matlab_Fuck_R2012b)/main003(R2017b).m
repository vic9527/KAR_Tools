...
%              clearvars -except All_Actions indices kk k T TT Cbag CaccuracyBag CPRPbag MeanCaccuracy Finaccuracy StdFA TTCaccuracy TTCbag TTCPRPbag TR_Actions vv vvindices TE_Actions
               ...
               train_v_ID=find(validation==1);
               train_t_ID=find(validation==0);
               TR_V_Actions= TR_Actions1C(train_v_ID);%分配验证集
               TR_T_Actions= TR_Actions1C(train_t_ID);%分配训练集
                ...
                %% using HMM
                 ...
                 HMM_Models = hmmTrain1C(TR_T_Actions, param);%tt

                 % Testing
                 fprintf('HMM testing.\n');
                 % prescale test data
                 [accuracy, predict_label1C, true_label] = hmmTest1C(TR_V_Actions, HMM_Models);%tt
                 
                 HMM_accuracy=accuracy
                 Vaccuracy_2nd(1,v)=HMM_accuracy(1);

                %% using LSTM
                 for jj=1:size(TR_V_Actions,2)
                 LSTM_Test_Y(jj,1)=TR_V_Actions(jj).label; 
                 LSTM_Test_X{jj,1}=TR_V_Actions(jj).Feature2nd(13:18,:);
                 end  
                 for jj=1:size(TR_T_Actions,2)
                 LSTM_Train_Y(jj,1)=TR_T_Actions(jj).label; 
                 LSTM_Train_X{jj,1}=TR_T_Actions(jj).Feature2nd(13:18,:);
                 end 
                 ...
                 LSTM_Test_Y=TR_LL_Trans;
                 
                
                 LSTM_predict=LSTM1C(LSTM_Train_X,LSTM_Train_Y,LSTM_Test_X,LSTM_Test_Y); 
                 ...
                 n = numel(u);
                 for Trans_cc=1:n
                    LSTM_predict(find(LSTM_predict==Trans_cc))=u(Trans_cc);
                 end
                 
                 LSTM_accuracy=sum(LSTM_predict == TR_LL)/size(TR_LL,1)
                 
                 Vaccuracy_2nd(2,v)=LSTM_accuracy(1);
                clearvars LSTM_Train_X LSTM_Train_Y LSTM_Test_X LSTM_Test_Y
          
               end
               
          Vaccuracy_2nd_1=Vaccuracy_2nd;     
          Cnum=2;%分类器数量
          Vaccuracy_2nd_1(:,v+2)=mean(Vaccuracy_2nd_1(1:Cnum,1:v),2);
          [C,I]=max(Vaccuracy_2nd_1)
          Vaccuracy_2nd_1(Cnum+2,:)=C;
          Vaccuracy_2nd_1(Cnum+3,:)=I;
          Cvote=tabulate(Vaccuracy_2nd_1(Cnum+3,1:v));
          [MaxCvote,S1stC]= max(Cvote(:,3));     %Select the first level classifier.
          
          CvoteMax_ID=find(Cvote(:,3)==MaxCvote);
          if length(CvoteMax_ID)~=1
              CvoteMeanMax=[CvoteMax_ID,Vaccuracy_2nd_1(CvoteMax_ID,v+2)];
              [MaxCvote_value,S1stC_index]=max(CvoteMeanMax(:,2));
              S1stC=CvoteMeanMax(S1stC_index,1);
          end
              S1stC_Mat(k)=S1stC;
              VaccuracyBag{k}=Vaccuracy_2nd_1;
              
          clearvars Vaccuracy_2nd_1
         ...
         %% 选择最优分类器      
          switch  S1stC
               ...
               %% 选择LSTM和分类器
                 for jj=1:size(TR_Actions,2)
                 LSTM_Test_Y(jj,1)=TR_Actions(jj).label; 
                 LSTM_Test_X{jj,1}=TR_Actions(jj).Feature2nd(13:18,:);
                 end  
                 for jj=1:size(TE_Actions,2)
                 LSTM_Train_Y(jj,1)=TE_Actions(jj).label; 
                 LSTM_Train_X{jj,1}=TE_Actions(jj).Feature2nd(13:18,:);
                 end 
                 ...
                 LSTM_accuracy=sum(LSTM_predict == TR_LL)/size(TR_LL,1)
                 S2ndC_predict= LSTM_predict;
                 
                 otherwise  
                      return;
               end
                 
                 predict_label1C=S2ndC_predict;
