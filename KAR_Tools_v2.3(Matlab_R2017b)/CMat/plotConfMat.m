...
confpercent0 = 100*confmat./repmat(sum(confmat, 1),numlabels,1);
confpercent1=roundn(confpercent0,-2);
 %对角线全为0
BZ=confpercent1;
% confpercent2=BZ+diag(-diag(BZ))
BZ(logical(eye(numlabels)))=0;

%更新对角线元素新值
BZ=diag(100-sum(BZ))+BZ;
...
textStrings = num2str([confpercent(:)], '%.2f%%\n');
...
set(gca,'XTick',1:numlabels,...
    ...
    'YTickLabel',labels,...
    'TickLength',[0 0]);
rotateXLabels(gca, 20 );% rotate the x tick  
end