...
%TT=10
for T=1:TT
[TTx(T),TTy(T)]=find(TTCaccuracy(:,T)==max(TTCaccuracy(:,T)));
end

% % % for T=1:TT
% % % FFFFF(T)=TTCaccuracy(TTx(T),T);
% % % end

for T=1:TT
TTCPRPbag_MAX(T)=TTCPRPbag(TTx(T),T);
end

TTCPRPbag_MAX_ALL=[];
for T=1:TT
TTCPRPbag_MAX_ALL=[TTCPRPbag_MAX_ALL;TTCPRPbag_MAX{T}(:,[1 4])];
end

% TTNumber=length(find(TTCPRPbag_MAX_ALL(:,1) == TTCPRPbag_MAX_ALL(:,2) ))
% TTNumber/size(TTCPRPbag_MAX_ALL,1) 

for T=1:TT
TTCPRPbag_MAX_SD(1,T)=length(find(TTCPRPbag_MAX{T}(:,1) ~=TTCPRPbag_MAX{T}(:,4)));
TTCPRPbag_MAX_SD(2,T)=length(find(TTCPRPbag_MAX{T}(:,1) ==TTCPRPbag_MAX{T}(:,4)));
end
TTCPRPbag_MAX_SD(3,:)=sum(TTCPRPbag_MAX_SD,1);
TTCPRPbag_MAX_SD(:,11)=sum(TTCPRPbag_MAX_SD,2);

TTCPRPbag_MAX_SD(4,:)=TTCPRPbag_MAX_SD(2,:)./TTCPRPbag_MAX_SD(3,:);
TTCPRPbag_MAX_SD(5,1)=mean(TTCPRPbag_MAX_SD(4,1:10));



TTActions=20;
for tt=1:TTActions
TTActionsNumber(1,tt)=length(find(TTCPRPbag_MAX_ALL(:,1) ==tt&TTCPRPbag_MAX_ALL(:,1) == TTCPRPbag_MAX_ALL(:,2) ));
TTActionsNumber(2,tt)=length(find(TTCPRPbag_MAX_ALL(:,1) ==tt));
end

% TTActionsNumber(3,:)=sum(TTActionsNumber,1);
TTActionsNumber(:,21)=sum(TTActionsNumber,2);
TTActionsNumber(3,:)=TTActionsNumber(1,:)./TTActionsNumber(2,:);
